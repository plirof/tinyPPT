...
v005h5 251022 : added media plugin , added CSS 
v005h4 251017 :
...
CDN:
<script src="https://cdn.jsdelivr.net/npm/tinymce@6.1.2/tinymce.min.js" referrerpolicy="origin"></script>
<!--
LOCAL:
...
<div id="top-controls">
  ...
  <select id="theme-selector">
    ...
    <option value="white_contrast_compact_verbatim_headers.css">White Contrast Compact Verbatim Headers</option>
...
tinymce.init({
  ...
  height: 300,
  //menubar: false,
  plugins: 'lists link image table code,media', //media plugin for video/mp3
...
function addVerticalSlide(slideIndex) {
  const slide = slides[slideIndex];

  if (!slide.verticalSlides) {
    slide.verticalSlides = [];
  }

  // Insert at beginning
  slide.verticalSlides.unshift({
...
// When rendering the slide list, ensure the button calls this function with the correct index
function renderSlideList() {
  const list = document.getElementById('slide-list');
  list.innerHTML = '';

  // Recursive function to create list items with indentation
  function createSlideItem(slide, indexPath = []) {
    const li = document.createElement('li');

    // Build a string path for the slide position (not strictly necessary)
    const indexStr = indexPath.join('.');

    // Create the main slide display
    ...
    editBtn.onclick = () => editSlide(indexPath);

    const deleteBtn = document.createElement('button');
    deleteBtn.textContent = 'Delete';
    deleteBtn.onclick = () => deleteSlide(indexPath);

    const addVerticalBtn = document.createElement('button');
    addVerticalBtn.textContent = 'Add Vertical';
    addVerticalBtn.onclick = () => addVerticalSlide(indexPath[0]);

    actionsDiv.appendChild(editBtn);
    actionsDiv.appendChild(deleteBtn);
    actionsDiv.appendChild(addVerticalBtn);

    li.appendChild(titleSpan);
    li.appendChild(actionsDiv);

    // Indent based on hierarchy level
    li.style.paddingLeft = `${20 * indexPath.length}px`;

    // Drag & Drop handlers
    li.ondragstart = (e) => {
      e.dataTransfer.setData('text/plain', JSON.stringify(indexPath));
    };
    li.ondragover = (e) => {
      e.preventDefault();
    };
    li.ondrop = (e) => {
      ...
      moveSlide(fromPath, indexPath);
    };

    list.appendChild(li);

    // Render nested vertical slides if any
    if (slide.verticalSlides && slide.verticalSlides.length > 0) {
      slide.verticalSlides.forEach((vSlide, vIndex) => {
        createSlideItem(vSlide, [...indexPath, vIndex]);
      });
    }
  }
...
function saveSlide() {
  ...
  // Save nested slides if any
  const slide = slides[index];
  if (slide.verticalSlides) {
      // Example: collect nested slides data from your UI
      // You would replace this with your actual method of editing nested slides
      // For instance, you might have a global array 'currentVerticalSlidesData' that you update when editing nested slides
      if (window.currentVerticalSlidesData) {
        slide.verticalSlides = window.currentVerticalSlidesData;
      } else {
        // Keep existing if no new data
        slide.verticalSlides = slide.verticalSlides || [];
      }
  }

  //slides[index] = { title, content, transition, bgcolor };
    slides[index] = { title, content, transition, bgcolor, verticalSlides: slide.verticalSlides || [] };
...
// Function to update the display of slides in Reveal.js, including nested vertical slides
function updateSlideDisplay() {
  ...
  slides.forEach((slide) => {
    ...
    section.setAttribute('data-transition', slide.transition || defaultTransition);
    if (slide.bgcolor) {
    ...
    // Render nested vertical slides if any
    if (slide.verticalSlides && slide.verticalSlides.length > 0) {
      slide.verticalSlides.forEach((vSlide) => {
        const vSection = document.createElement('section');
        vSection.setAttribute('data-transition', vSlide.transition || defaultTransition);
        if (vSlide.bgcolor) {
          vSection.setAttribute('data-background-color', vSlide.bgcolor);
        }
        vSection.innerHTML = '<h3>' + vSlide.title + '</h3>' + vSlide.content;
        section.appendChild(vSection);
      });
    }
